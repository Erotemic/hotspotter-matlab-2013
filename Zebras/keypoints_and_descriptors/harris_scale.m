...
global radius_per_scale; %This maybe shouldn't be global
radius_per_scale = 6; 
...
if enable_progress
  progress_handle = waitbar(0,'Harris Scale: Computing Scale Space');
end
ScaleSpace = compute_scale_space(I,'display_off');
if enable_progress
  waitbar(.1,progress_handle,'Harris Scale: Computing Harris Keypoints');
end
[harris_keypoints] = harris_keypoint_detector(ScaleSpace,'display_off','nonmax_radius',4,'nonmax_thresh',12);
...
if enable_progress
  waitbar(.4, progress_handle, 'Harris Scale: Computing Scales');
end
...
%Put keypoints back in original image coordinates
k(1,:) = k(1,:) /ScaleSpace.scale_factor;
k(2,:) = k(2,:) /ScaleSpace.scale_factor;
k(3,:) = k(3,:) /ScaleSpace.scale_factor;

ScaleSpace.scale_factor
