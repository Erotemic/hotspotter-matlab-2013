...
function imageArray = DisplayImage(handles, FullImageFileName)
	...
	if strcmp(extension, '.iri') == 1
		% Read in image from disk into an array.
		%imgOriginal = imread([handles.ImageFolder '\' ImageName]);
		imageArray=ReadIRIFile(FullImageFileName);
	elseif strcmp(extension, '.cew') == 1
		% Read in image from disk into an array.
		imageArray=ReadCEWFile(FullImageFileName);
	else	% e.g. TIF or BMP
