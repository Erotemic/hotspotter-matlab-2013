...
if exist('sample_computed','file')
  load('sample_computed');
else
  sample_computed = cell(sample_size,sample_size);
end

hist_same = -ones(sample_size,sample_size);
hist_diff = -ones(sample_size,sample_size);

images_to_score = 1:sample_size;
%images_to_score  = 166:175;
%images_to_score = [81 30];
...
for i = images_to_score
  ...
  for j = images_to_score
    ...
    [~,results] = score_zebras(i,j);
    sample_computed{i,j} = results;

    if ground_truth(i,field_data.name_index) == ground_truth(j,field_data.name_index)
    %Do same image stuff
      eval(['hist_same(i,j) = ' hist_experiment_var ';']);
      hist_diff(i,j) = -1;
    else
    %Do different image stuff
      eval(['hist_diff(i,j) = ' hist_experiment_var ';']);
      hist_same(i,j) = -1;
    end
  end
  save('sample_computed.mat','sample_computed');
  %figure(1);
  %hist(hist_same(hist_same ~= -1),100);
  %title('same')
  %figure(2);
